...
```{r, echo=FALSE}
### settings
s <- 6             ## Focal site 6 for forward simulation
...
ppt.mean <- matrix(apply(ppt_ensemble,2,mean),1,NT) ## driver i.e. rain/precpitation
## parameters
...
N.det <- forecastN(IC=mean(IC[,"N[6,30]"]), #IC for site 6
                   r=param.mean["r_global"],
                   Kg=param.mean["K_global"],
                   alpha=param.mean["alpha_site[6]"], # site 6 mean
...
```{r}
# actual parameter conditions rather than taking the mean of the parameters
...
```{r,echo=FALSE}
# Forecast of rain over time (30 time steps)
...
N.IPD <- forecastN(IC=IC[prow,"N[6,30]"],  ## sample IC
                   ...
                   alpha=params[prow,"alpha_site[6]"],
                   beta=params[prow,"beta"],
                   ppt=ppt_ensemble[drow,],   ## Sample drivers. Drawing
                   Q=0,
                   n=Nmc)
...
lines(time2,N.I.ci[2,],lwd=0.5)
```
black CI - Initial condition uncertainty (only small from 30-35)
Red = parameter uncertainty
Green = rainfall uncertainty progagation
...
N.IPDE <- forecastN(IC=IC[prow,"N[6,30]"],  ## sample IC
                   ...
                   Q=Qmc, # adding process uncertainty
                   n=Nmc)
...
Green = rainfall uncertainty progagation
blue = process uncertainty
orange = Random effect uncertainty. see difference b/w sites from top fig (yellow site)
