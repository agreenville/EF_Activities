...
This activity will explore the state-space framework for modeling time-series and spatial data sets. Chapter 8 provides a more in-depth description of the state-space model, but in a nutshell it is based on separating the process model, which describes how the s2ystem evolves in time or space, from the observation error model. Furthermore, the state-space model gets its name because the model estimates that true value of the underlying **latent** state variables.
...
n<-data$n
names(data_missing)
data_missing$y[580:620] <- NA
```

We already defined the RandomWalk model above and we are not changing the model at this step. We also already defined the data and priors as a list on line 54, including logging the data, so we don't need to read the data in again. 
...
ecoforecastR::ciEnvelope(time,ci[1,],ci[3,],col=ecoforecastR::col.alpha("lightBlue",0.75))
points(time[1:580],y[1:580],pch="+",cex=0.5)
points(time[581:620],y[581:620],pch="+",cex=0.5, col="red")
```
