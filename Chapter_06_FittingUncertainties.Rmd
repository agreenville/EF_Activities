...
GBR.burn <- gelman.plot(var.burn)

## convert to matrix
var.mat.burn      <- as.matrix(var.burn)

## Pairwise scatter plots & correlation
pairs(var.mat.burn)	## pairs plot to evaluate parameter correlation
cor(var.mat.burn)    ## correlation matrix among model parameters
...
### Task 1.2:
```{r}
xpred <- seq(min(TDR), by=0.1) #sort(TDR)
plot(TDR,y)
# use exp for log link to tranform back to response scale
for(i in 1:10){
  lines(xpred, exp(var.mat[i,"beta[1]"] + var.mat[i,"beta[2]"]*xpred))
}
...
xpred <- seq(min(TDR), by=0.1) #sort(TDR)  					## sequence of x values we're going to
...
for(g in seq_len(nsamp)){
  ...
  #note exp to backtransform to response scale
...
plot((TDR),y,cex=0.5,xlim=c(min(TDR),max(TDR)),ylim=c(0,35))
